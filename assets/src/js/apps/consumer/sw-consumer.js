...
self.addEventListener('notificationclick', (event) => {
    ...
    event.waitUntil(
        self.clients // CORREÇÃO: Usando self.clients
            .matchAll({
                type: 'window',
            ...
            .then((clientList) => {
                ...
                if (self.clients.openWindow) {
                    // CORREÇÃO: Usando self.clients
                    console.log(
                        '[SW DEBUG] Nenhum cliente existente. Abrindo nova janela para:',
                        targetUrl
                    );
                    return self.clients.openWindow(targetUrl); // CORREÇÃO: Usando self.clients
                }
            })
    );
});
