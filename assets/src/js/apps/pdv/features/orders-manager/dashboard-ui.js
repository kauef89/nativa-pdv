// apps/pdv/features/orders-manager/dashboard-ui.js

import { showModal } from '@ui/modals/modal.js';
import { formatAddress, formatPrice } from '@utils/formatters.js';
import { escapeHTML } from '@utils/ui-helpers.js';
import { state } from './dashboard-state.js';

let timeAgoInterval = null;
// CORREÇÃO: Removido 'BATCH_SIZE' não utilizado
                ...
                .map((item) => {
                    ...
                    if (item.is_combo && Array.isArray(item.selections)) {
                        ...
                        item.selections.forEach((sel) => {
                            ...
                            // CORREÇÃO: Usando Object.values para evitar 'no-unused-vars' em groupId
                            if (
                                sel.selectedAddons &&
                                ...
                                Object.values(sel.selectedAddons).forEach(
                                    (group) => {
                                        if (
                                            group &&
                                            group.items &&
