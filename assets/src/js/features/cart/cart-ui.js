/**
 * Módulo responsável por todas as manipulações do DOM no carrinho.
 * Funções extraídas de módulos globais para melhorar a modularidade.
 * ATUALIZAÇÃO (UI): Adiciona uma classe ao contêiner de endereço do usuário quando a mensagem de frete grátis é exibida, permitindo estilização condicional do border-radius.
 * ATUALIZAÇÃO (UI): Adiciona classe 'is-disabled' aos pills de endereço quando a entrega está indisponível.
 * CORREÇÃO (Render Addons): Adiciona verificação para garantir que o objeto 'addon' exista antes de acessar suas propriedades em _renderAddonListItems.
 */
...
const _renderAddonListItems = (addons) => {
    ...
    if (!addons || typeof addons !== 'object') {
        // Verifica se addons é um objeto válido
        return addonsList;
    }

    for (const groupId in addons) {
        // Verifica se o grupo e os itens existem e são objetos
        if (
            ...
            for (const addonItemIndex in addons[groupId].items) {
                ...
                // --- INÍCIO DA CORREÇÃO ---
                // Verifica se 'addon' é um objeto válido antes de acessar suas propriedades
                if (addon && typeof addon === 'object' && addon.itemName) {
                    ...
                    const quantityPrefix =
                        addon.itemQuantity > 1
                            ? `${addon.itemQuantity} × `
                            : '';
                    const addonPrice = addon.final_cost ?? addon.itemPrice; // Usa final_cost se existir
                    ...
                    // Loga um aviso se um item de addon inválido for encontrado
                    console.warn(
            ...
            // Loga um aviso se a estrutura do grupo for inválida
            console.warn(
...
const _renderCartItemDetails = (item) => {
    if (item.is_combo && Array.isArray(item.selections)) {
        const selectionsHtml = item.selections
            .map((selection) => {
                // Passa os addons da seleção para _renderAddonListItems
    ...
    // Passa os addons do item para _renderAddonListItems
...
const _renderCartItemActions = (key, item, isCheckout) => {
    if (isCheckout) return ''; // Sem ações na página de checkout

    // Botão de editar só aparece se NÃO for item de oferta ou recompensa
    const editButton =
...
const _renderCartItems = (cartContents) => {
    ...
    container.innerHTML = ''; // Limpa completamente
    ...
    for (const key of cartKeys) {
        ...
        // Adiciona uma verificação extra para garantir que itemData é válido
        if (itemData && typeof itemData === 'object') {
            const newItemHTML = renderCartItemHTML(itemData, key, false); // Passa false para isCheckout
...
const _renderUserAddress = (addresses, bairros) => {
    ...
    // Tenta usar o endereço salvo na sessão primeiro
    ...
    // Se não encontrou na sessão, usa o primário ou o primeiro
    if (!addressToDisplay && addresses.length > 0) {
        addressToDisplay =
            addresses.find((addr) => addr.is_primary) || addresses[0];
        // Atualiza a sessão se um endereço foi selecionado por padrão
        if (addressToDisplay) {
    ...
    // Atualiza o bairro selecionado no estado global
    if (addressToDisplay) {
        state.selectedBairro = state.allBairros.find(
            (b) => b.id == addressToDisplay.bairro_id
        );
    } else {
        state.selectedBairro = null; // Limpa se nenhum endereço for exibido
    }
...
// renderAddressSelectionSheet (sem alterações)
...
// renderCartItemHTML (sem alterações, a correção está em _renderAddonListItems)
...
// renderCartUI (sem alterações)
export const renderCartUI = (cartData, canCheckout, serviceStatus) => {
    _renderCartItems(cartData.cart_contents);
    // As outras partes (resumo financeiro, modalidade, etc.) são atualizadas por funções separadas
};

// updateFinancialSummary (sem alterações)
...
// updateFreteGratisMessage (sem alterações)
export const updateFreteGratisMessage = (
    ...
    const parentContainer = messageEl.closest(
        '#nativa-cart-user-address-container, #nativa-cart-guest-bairro-container' // Verifica ambos os containers
    );
...
// populateBairrosSelect (sem alterações)
    ...
    if (bairros && bairros.length > 0) {
        ...
        const pillsHtml = bairros
            .map((bairro) => {
                ...
                // Verifica se o bairro está disponível para entrega (futura implementação, se necessário)
                const isAvailable = true; // Por enquanto, assume que todos estão disponíveis
...
// updateModalityDisplay (sem alterações)
export const updateModalityDisplay = (
    ...
    let isDeliveryAvailable = false; // Flag para verificar a disponibilidade da entrega
    ...
    const guestPillsContainer = guestContainer?.querySelector(
        // Seleciona o container de pills do convidado
    ...
    if (selectedModality === 'delivery') {
        if (isLoggedIn) {
            ...
            if (userAddressContainer) {
                ...
                _renderUserAddress(addresses, bairros); // Re-renderiza a lista de endereços
                updateFreteGratisMessage(
                    // Atualiza msg de frete
                    state.cart.subtotal,
                    state.selectedBairro,
                    selectedModality
                );
                // Aplica classe 'is-disabled' aos pills de endereço se necessário
            ...
            if (guestContainer) {
                ...
                // Re-renderiza a lista de bairros (pode ter sido atualizada)
                ...
                // Aplica classe 'is-disabled' aos pills de bairro se necessário
...
// updateCartDisplay (sem alterações)
