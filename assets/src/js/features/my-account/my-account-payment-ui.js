/**
 * Módulo de UI para renderizar e gerenciar todos os componentes
 * relacionados a pagamentos pendentes na página "Minha Conta".
 * Refatorado para a arquitetura de módulos ES6.
 *
 * ATUALIZAÇÃO (Fluxo PIX v3):
 * 1. Adiciona contagem regressiva (timer) aos blocos pix-manual e pix-fallback.
 * 2. Padroniza os títulos dos blocos de pagamento para "Aguardando seu pagamento".
 * 3. Adiciona um subtítulo informativo.
 * ATUALIZAÇÃO (QR Code Refactor):
 * 1. A função _renderAutomaticPixUI agora é 'async'.
 * 2. Importa e chama 'getPixData' da API para buscar o QR code sob demanda,
 * em vez de esperar que ele venha no objeto 'order'.
 * 3. Adiciona tratamento de erro (try/catch) para a chamada da API, renderizando
 * o fallback em caso de falha.
 * ATUALIZAÇÃO (PUSH PIX):
 * 1. Importa 'sendPixExpirationWarning' da API.
 * 2. Adiciona lógica em '_startProgressBar' para chamar a API quando o timer atingir 60 segundos.
 */

import { copyToClipboard, showToast } from '../../utils/nativa-ui-helpers.js';
