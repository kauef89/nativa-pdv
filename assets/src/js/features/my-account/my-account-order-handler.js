// js/features/my-account/my-account-order-handler.js

/**
 * Módulo para lidar com as ações do usuário relacionadas a pedidos na página "Minha Conta".
 * Refatorado para a arquitetura de módulos ES6.
 * ATUALIZAÇÃO (Recálculo Total): Recalcula o total na ficha "Pedir Novamente" usando os preços atuais dos produtos e adicionais.
 * CORREÇÃO (Modal Cancelar): Inverte a lógica do modal de cancelamento para corresponder à ação do botão.
 * NOVO (PWA Check): Adiciona verificação para exigir instalação PWA antes de adicionar itens de pedidos anteriores ao carrinho.
 * ATUALIZAÇÃO (Reorder Offers/Rewards): Processa itens de oferta e recompensa corretamente ao adicionar ao carrinho.
 */
...
} from '../../utils/nativa-utils.js'; // Importa formatNumberWithThousandSeparator
...
function _updateReorderSheetTotal() {
    ...
    try {
        ...
        const allProducts = state.menu.products || []; // Usa state.menu.products
    ...
    if (
        ...
        try {
            ...
            if (!state.user.rewards) {
                const rewardsData = await api.getAvailableRewards(); // Supondo que api está importado
