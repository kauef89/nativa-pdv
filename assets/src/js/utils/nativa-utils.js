// js/utils/nativa-utils.js

/**
 * Contém funções de utilidade puras e globais.
 * Refatorado para a arquitetura de módulos ES6.
 * ATUALIZAÇÃO: Adiciona 'escapeHTML' para quebrar dependências circulares.
 */
...
export const isWithinSchedulingWindow = (operatingHours) => {
    ...
    if (todaysHours && todaysHours.is_active === 'on') {
        const [openHour, openMinute] = todaysHours.store_hours.open
            .split(':')
            .map(Number);
